...
define(__NAMESPACE__ . "\DIRECTORY_PATH", "X:\\services\\websvr\\cse442-project\\html\\db");
...
function LOG($type, $message){
	...
	//	Include the name of the calling function if debug-style logging is enabled.
	if (LOG_DEBUG_FORMAT) $line = "[" . $timestamp . "] [" . $_SERVER['REMOTE_ADDR'] . "] [" . $type . "] [DEBUG @" . GET_CALLER_FUNCTION() . "]: " . $message;
	else $line = "[" . $timestamp . "] [" . $_SERVER['REMOTE_ADDR'] . "] [" . $type . "]: " . $message;
	
	//	Echo the entry to the screen if log-echo feature is enabled.
	if (ENABLE_LOG_ECHO) echo "<pre>" .  $line . "</pre>";
...
function DB_INIT(){
	...
	LOG("INFO", "Starting " . $operation_name . "...");
	
	try{
		
		// Open or create database if it doesn't exist.
		LOG("INFO", "Opening/creating database \"" .  DB_NAME  .  "\"...");
		$db = new \PDO('sqlite:' . DB_NAME);
		
		//	Create 'users' table.
		$tableName = "users";
		LOG("INFO", "Creating \"" . $tableName . "\" table...");
		...
		if ($result === false || $result === 0){
			...
			//	Success with warning.
				...
				if (isset($error[1]) && isset($error[2]))
					LOG("WARNING", "Success (DB WARNING: [Code: " . $error[1] . "] [Message: " . $error[2] . "]." );
				else
					LOG("WARNING", "Success (DB WARNING: [Database reported success with warning, but failed to return further information].");
			}
			...
			//	All other errors.
				...
				$error_count++;
				if (isset($error[1]) && isset($error[2]))
					LOG("ERROR", "Failed (DB ERROR: [Code: " . $error[1] . "] [Message: " . $error[2] . "]." );
				else
					LOG("ERROR", "Failed (DB ERROR: [Database reported error, but failed to return further information]." );
			}
		}
		...
		$db = null;
		// isset($db) ? LOG("ERROR", "Failed to close database connection.") : LOG("INFO", "Successfully closed database connection.");
		if (isset($db)){
			$error_count++;
			LOG("ERROR", "Failed to close database connection.");
		}
		else LOG("INFO", "Successfully closed database connection.");
		
		LOG("INFO", "Finished " . $operation_name . ".");
		// LOG("INFO", " --- " . $warning_count . " Warning(s)");
		// LOG("INFO", " --- " . $error_count . " Error(s)");
		...
		$error_count++;
		LOG("INFO", "============ " . $operation_name . ": " . $success_count . " succeeded, " . $error_count . " failed, " . $warning_count . " warning(s) ============");
		return 1;
	}
